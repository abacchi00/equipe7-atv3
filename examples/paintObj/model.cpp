...
void Model::loadObj(std::string_view path, bool standardize) {
  ...
  for (auto const &shape : shapes) {
    ...
    for (std::size_t offset = 0; offset < shape.mesh.indices.size(); ++offset){
      ...
      if (index.texcoord_index < 0) {
        // Adicione coordenadas UV padrão
        texCoord = glm::vec2((offset % 3 == 0) ? 0.0f : 1.0f, (offset % 3 == 2) ? 0.0f : 1.0f);
      }
      ...
      // Sempre insira novos vértices, mesmo que tenham posições iguais
